...
class FacialGNN(nn.Module):
    ...
    --- MODIFIED VERSION ---
    This version uses a *unique* CNN for *each* node (patch) in the graph.
    """
    
    # --- CHANGED: Added 'num_nodes' to the constructor ---
    def __init__(self, num_nodes, input_channels, feature_dim=64, gcn_hidden_dim=128, output_dim=2):
        ...
        # --- CHANGED: Replaced the single CNN with a ModuleList of CNNs ---
        # self.cnn_feature_extractor = CNNFeatureExtractor(input_channels, feature_dim) # <-- OLD
    ...
    def forward(self, data, return_activations=False):
        ...
        # --- CHANGED: Node Feature Extraction ---
        ...
        # This logic is inspired by 'record_image_activations' in utils.py
        local_indices = torch.arange(len(batch), device=x.device) - data.ptr[batch]

        # 2. Create empty tensors to store the results
        # We must ensure the results are in the *original order* for the GCN
        ...
        # --- CHANGED: Kernel and Activation handling ---
